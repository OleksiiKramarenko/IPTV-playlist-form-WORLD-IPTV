...
# Чёрный список ключевых слов — если group-title СОДЕРЖИТ любое из них,
# канал отсекается. Всё остальное (включая неизвестные группы) — проходит.
EXCLUDE_KEYWORDS = [
    # Страны и регионы (достаточно корня слова)
    "австрали", "australia",
    "австрия", "austria",
    "бельгия", "belgium",
    "азербайджан", "azerbaijan", "азери",
    "албани",
    "армени", "հայկական",
    "арабск", "arabic", "arab",
    "африк", "africa",
    ...
    "болгари", "bulgaria",
    "бразили", "brasil",
    "великобритани", "united kingdom",
    "венгри", "hungary",
    "германи", "germany", "deutsch",
    "греци", "greece",
    "грузи", "georgia",
    "дани", "denmark",
    "египет", "egypt",
    ...
    "татарст",
    "туркменист",
    "турецк", "турци", "turkish", "türk",
    "узбекист", "узбекск", "uzbekistan",
    "украін", "україн", "ukraine",
    "финлянд", "finland",
    "франци", "france",
    "хорвати", "croatia",
    "чехи", "czech",
    ...
    # Радио
    "радио", "radio",
    # Региональное / операторское
    "региональн", "телемагазин", "оплот",
    "vpn", "торрент тв",
    # Технический мусор и провайдеры
    "test", "удалит", "тест",
    "cloudflare", "webhost", "agronet", "селектел",
    "квант-телеком", "цитадель", "бегет", "nexo", "olsib", "nnt",
    "тт мобайл", "turon", "евразия-стар", "catcast", "claruty",
    "internet42", "itv.uz", "сириус",
    # Сериалы и play-x
    "play-x", "сериал",
    # Прочее
    "кухн", "хобби кухн",
    "общест", "сайт",
    "webcam",
    "playlist info", "плейлист тормоз", "дата обнов", "web", "live", "другое", "undefined", "cloud", "дата", "страны", "эфир",
    "страны", "федеральн",  # агрегаторные группы
]
# ─────────────────────────────────────────────────────────────────────────────
...
def is_excluded(meta: str) -> bool:
    """
    Чёрный список: отсекаем если group-title содержит
    хотя бы одно ключевое слово из EXCLUDE_KEYWORDS.
    Каналы без group-title или с неизвестной группой — оставляем.
    """
    m = re.search(r'group-title="([^"]*)"', meta, re.IGNORECASE)
    if not m:
        return False  # нет группы — оставляем
    group = m.group(1).strip().lower()
    if not group:
        return False  # пустая группа — оставляем
    return any(kw in group for kw in EXCLUDE_KEYWORDS)
